...
<body>
    ...
    <h4>Stammdaten verwalten</h4>
		...
		<form action="<?= base_url('masterdata/update') . "/$id" ?>" method="post" >
			<?= csrf_field(); ?>
			<div class="form-group">
				<label for="">Name der Firma</label>
				<input type="text" class="form-control" name="name_orga" value="<?= $name_orga ?>" />
			...
			<div class="form-group">
				...
				<input type="text" class="form-control" name="email_contact" value="<?= $email_contact ?>" />
				<span class="text-danger"> <?= isset($validation) ? display_error($validation, 'email_contact') : '' ?></span>
			</div>

			<div class="form-group">
				<button class="btn btn-primary btn-block" type="submit">Datensatz aktualisieren</button>
			</div>
